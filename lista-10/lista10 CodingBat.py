...
def cat_dog(s):
  return True if s.count("cat") == s.count("dog") else False
...
def count_code(s):
  ...
  casos = 0
  for letra in letras:
    if f"co{letra}e" in s:
      casos += s.count(f"co{letra}e")
  return casos
...
def end_other(a, b):
  if a.lower() in b.lower(): 
    if a.lower() in b[len(a)*-1:] in b.lower():
      return True
    else:
      return False
  elif b.lower() in a.lower():
    if b.lower() in a[len(b)*-1:] in a.lower():
      return True
    else:
      return False
  else:
...
def has22(nums):
  ...
  for n in nums:
    s += str(n)
  return True if "22" in s else False
